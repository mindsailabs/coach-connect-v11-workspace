...
const NeumorphicJourneyDesignerV9 = ({
  ...
  const timelineRef = useRef(null); // Middle notches timeline
  const stepTimelineRef = useRef(null); // Bottom steps timeline
  const sessionTimelineRef = useRef(null); // Top sessions timeline
  ...
  const isDateOccupied = useCallback((date, excludeId = null) => {
    const allItems = [...sessions, ...steps];
    
    return allItems.some((item) => {
      // Only exclude the item being moved
      if (item.id === excludeId) return false;

      // Exclude default sessions from collision checks
      if (item.type === 'session' && item.isDefault) return false;

      // Check date
      return item.date && compareDates(item.date, date);
    });
  }, [sessions, steps, compareDates]);

  const getItemOnDate = useCallback((date, excludeId = null) => {
    const allItems = [...sessions, ...steps];
  ...
  const snapToAvailableDate = useCallback((position, itemId, itemType = 'session') => {
    ...
    if (!isDateOccupied(targetDate, itemId)) { // itemType no longer needed here due to updated isDateOccupied
    ...
    for (let offset = 1; offset <= 7; offset++) {
      ...
      if (!isDateOccupied(beforeDate, itemId)) { // itemType no longer needed
      ...
      if (!isDateOccupied(afterDate, itemId)) { // itemType no longer needed
  ...
  // Middle timeline no longer creates sessions
  const handleTimelineClick = useCallback((e) => {
    return;
  }, []);

  // New handler for sessions timeline
  const handleSessionTimelineClick = useCallback((e) => {
    if (isDragging || e.target.closest('.session-item')) return;
    
    const rect = sessionTimelineRef.current.getBoundingClientRect();
  ...
  const handleDragStart = useCallback((e, item) => {
    ...
    const updateItemPosition = (item, viewPos, viewIndex, daysPerViewForDrag) => {
      ...
      // Snap to available date to avoid collisions
      const maxAllowedPos = item.type === 'session' ? 99 : 98;
      const rawPosition = Math.max(0, Math.min(maxAllowedPos, globalPos));
      const snappedPosition = snapToDay(rawPosition); // First snap to day
      const newPosition = snapToAvailableDate(snappedPosition, item.id, item.type); // Then find available date
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      const timelineEl = item.type === 'session' ? sessionTimelineRef.current : stepTimelineRef.current;
  ...
  const updateSelectedItem = useCallback((field, value) => {
    ...
    if (field === 'date') {
      ...
      const occupyingItem = getItemOnDate(value, selectedItem.id); // itemType no longer needed here
      if (occupyingItem) {
  ...
  const renderSessions = () => {
    ...
    return sessions.map((session) => {
      ...
      // Handle Start flag - keep existing logic
      ...
      // Skip default sessions if not in their view (and not the special start/end already handled)
      if (session.isDefault && ((isStart && currentViewIndex !== 0) || (isEnd && currentViewIndex !== totalViews - 1))) {
        return null;
      }
      ...
      // Handle End flag positioning
      if (isEnd && currentViewIndex === totalViews - 1) {
        // Don't use a percentage - we need to handle this specially
        displayPos = null; // We'll handle this with a special case below
      }

      if (displayPos === null && !(isEnd && currentViewIndex === totalViews - 1)) return null;

      // Special handling for End flag position
      if (isEnd && currentViewIndex === totalViews - 1) {
        return (
          <div
            key={session.id}
            ...
            style={{
              left: isPartialView 
                ? `calc(${timelineWidthPercent}% + 30px)` // 30px into the gap
                : `calc(100% + 30px)`, // 30px past full timeline
              transform: 'translateX(-50%)',
              top: '50%',
              marginTop: '-24px',
              cursor: 'default',
      ...
      return (
        <div
          ...
          style={{
            ...
            transform: 'translateX(-50%)',
            top: '50%',
            marginTop: `-${Math.max(16, Math.min(24, 24 * getIconScale()))}px`,
          ...
          onMouseDown={(e) => handleDragStart(e, session)}
          onMouseEnter={() => setHoveredItem(session.id)}
          onMouseLeave={() => setHoveredItem(null)}
          onClick={() => !isDragging && setSelectedItem(session)}
        >
          <div
          ...
          >
            <SessionIcon 
              style={{
                width: `${Math.max(16, Math.min(24, 24 * getIconScale()))}px`,
                height: `${Math.max(16, Math.min(24, 24 * getIconScale()))}px`,
                color: '#2f949d',
                fill: 'none'
              }}
            />
  ...
  return (
    <NeumorphicCard className="space-y-6">
      ...
      <div className="transition-opacity duration-300">
        <div className="relative px-20 mb-4"> {/* Sessions timeline */}
          ...
          <div className="relative">
            <div
              ...
              onClick={handleSessionTimelineClick}
            ...
            {isPartialView && (
              <div
                ...
                style={{
                  left: `calc(${timelineWidthPercent}% + ${Math.max(150, timelineWidthPercent * 3)}px)`,
                  width: `calc(${100 - timelineWidthPercent}% - ${Math.max(150, timelineWidthPercent * 3)}px)`,
                  background: 'var(--nm-background)',
                  opacity: 0.4,
                  boxShadow: 'var(--nm-shadow-inset)',
                  pointerEvents: 'none'
        ...
        <div className="relative px-20 mb-4"> {/* Middle notches timeline */}
          {currentViewIndex > 0 && (
            <div
          ...
          <div className="relative">
            ...
            >
              {/* Start flag - now rendered by renderSessions */}
              {/* End flag - now rendered by renderSessions */}
              
              {/* Test marker for correct end position (for debug only, will remove) */}
              {currentViewIndex === totalViews - 1 && (
                <div
                  className="absolute flex flex-col items-center"
                  style={{
                    // Position in the middle of the gap
                    // Gap starts at timelineWidthPercent% and has width of Math.max(150, timelineWidthPercent * 3)px
                    left: isPartialView 
                      ? `calc(${timelineWidthPercent}% + ${Math.max(150, timelineWidthPercent * 3) / 2}px)`
                      : `calc(100% + 75px)`, // For full view (no inactive section)
                    transform: 'translateX(-50%)',
                    top: '50%',
                    marginTop: '-30px',
                    zIndex: 100
                ...
                >
                  ...
                  />
                  <div className="text-xs font-bold text-green-500 mt-1">GAP</div>
                </div>
              )}
              ...
              {/* Session notches on top edge */}
              {sessions.filter(s => !s.isDefault).map((session) => {
                const viewPos = globalToViewPosition(session.position);
                const outOfBoundsAtEnd = sessions.filter((s) =>
                  outOfBoundsSessions.has(s.id) && s.position === 99
                );

                let displayPos = viewPos;
                if (outOfBoundsSessions.has(session.id) && session.position === 99 && currentViewIndex === totalViews - 1) {
                  const index = outOfBoundsAtEnd.findIndex((s) => s.id === session.id);
                  if (index !== -1) {
                    const spacing = 13 / Math.max(1, outOfBoundsAtEnd.length);
                ...
                return (
                  <div
                    key={`session-notch-${session.id}`}
                    className="absolute top-0 w-0.5 h-3"
                    style={{
                      left: `${displayPos}%`,
                      transform: 'translateX(-50%)',
                      background: outOfBoundsSessions.has(session.id) ? '#ef4444' : '#4299e1',
                      opacity: 0.8
                    }}
                  />
                );
              ...
              {steps.map((step) => {
                ...
                return (
                  <div
                    key={`step-notch-${step.id}`}
                    className="absolute bottom-0 w-0.5 h-3"
                    style={{
                      left: `${displayPos}%`,
                      transform: 'translateX(-50%)',
                      background: outOfBoundsSteps.has(step.id) ? '#ef4444' : '#48bb78',
                      opacity: 0.8
                    }}
                  />
                );
        ...
        <div className="relative px-20"> {/* Steps timeline */}
