...
export default function AssignNotePanel({ open, onClose, onAssign, currentAssignment }) {
  // Fetch contacts
  ...
  // currentAssignment: { type: 'contact'|'journey'|'session', id: string, contactId?: number }
  const [selectedContact, setSelectedContact] = useState(null);
  const [viewMode, setViewMode] = useState(null); // null | 'allContacts' | 'allSessions' | 'allJourneys'
  ...
  // Find the current contact from assignment
  ...
  const title = viewMode === 'allContacts' ? 'Contacts' : viewMode === 'allSessions' ? 'Sessions' : viewMode === 'allJourneys' ? 'Journeys' : selectedContact ? selectedContact.name : 'Reassign Note To';
  const backLabel = (viewMode || selectedContact) ? 'Back to Reassign Note' : 'Back to Note';
                  ...
                  {contacts.filter(c => c.sessions.length > 0).map((contact) => (
                    <div key={contact.id}>
                      <div className="flex items-center gap-2 mb-2 px-1">
                        <div
                          className="neumorphic-avatar sm flex-shrink-0"
                          style={{ backgroundColor: contact.color, color: '#fff', width: '1.25rem', height: '1.25rem', fontSize: '0.5rem' }}
                        >
                          ...
                          {contact.sessions.map((session) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className="neumorphic-avatar sm flex-shrink-0"
                                  style={{ backgroundColor: session.color, color: '#fff' }}
                                >
                    ...
                    <div className="divide-y" style={{ borderColor: 'rgba(209, 217, 230, 0.4)' }}>
                      ...
                      {currentContact && (
                        ...
                        >
                          <div
                            className="neumorphic-avatar sm flex-shrink-0"
                            style={{ backgroundColor: currentContact.color, color: '#fff' }}
                          >
                      ...
                      >
                        <div
                          className="neumorphic-avatar sm flex-shrink-0"
                          style={{ backgroundColor: 'var(--nm-badge-default-color)', color: '#fff' }}
                        >
                          <FileText className="w-3.5 h-3.5" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <span className="text-base font-normal">General Note</span>
                          <p className="text-xs" style={{ color: 'var(--nm-badge-default-color)' }}>
                      ...
                      >
                        <div
                          className="neumorphic-avatar sm flex-shrink-0"
                          style={{ backgroundColor: '#2f949d', color: '#fff' }}
                        >
                      ...
                      >
                        <div
                          className="neumorphic-avatar sm flex-shrink-0"
                          style={{ backgroundColor: '#ed8936', color: '#fff' }}
                        >
                      ...
                      {/* Individual contact cards removed - use Contact item above */}
                    </div>
                  </NeumorphicCard>
                </div>
              ) : (
                /* Step 2: Assign to contact or their journeys/sessions */
                <div className="space-y-4">
                  {/* Assign directly to contact */}
                  <NeumorphicCard 
                    clickable 
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className="neumorphic-avatar smPlus flex-shrink-0"
                        style={{ backgroundColor: selectedContact.color, color: '#fff' }}
                      >
                  ...
                  {/* Empty state if no journeys or sessions */}
