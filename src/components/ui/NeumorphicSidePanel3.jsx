...
const NeumorphicSidePanel3 = ({ isOpen, onClose, title, children, side = 'right' }) => {
  // Handle escape key
  React.useEffect(() => {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50" style={{ padding: 0, margin: 0 }}>
          ...
          <motion.div
            ...
            className={`absolute ${side === 'right' ? 'right-0' : 'left-0'} top-0 h-full w-full md:w-[80%] flex flex-col`}
            style={panelStyle}
          >
            {/* Header */}
            <div
...
export default NeumorphicSidePanel3;